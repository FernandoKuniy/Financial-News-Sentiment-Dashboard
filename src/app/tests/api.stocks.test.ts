...
describe("/api/stock", () => {
  ...
  it("daily series normalized", async () => {
  ...
  it("intraday key detection", async () => {
    (fetch as unknown as Mock).mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          "Time Series (30min)": {
  ...
  it("upstream error surfaced", async () => {
    (fetch as unknown as Mock).mockResolvedValueOnce(
      new Response(
        JSON.stringify({ Note: "Thank you for using Alpha Vantage! Please try again" }),
        { status: 200 }
      )
    );
    const res = await GET(mkReq("http://test/api/stock?symbol=AAPL"));
